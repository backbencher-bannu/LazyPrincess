...
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1989461858').split()]
...
# PREMIUM ACCESS *
lazydownloaders = [int(lazydownloaders) if id_pattern.search(lazydownloaders) else lazydownloaders for lazydownloaders in environ.get('PRIME_DOWNLOADERS', '1989461858').split()]
PRIME_USERS = (lazydownloaders) if lazydownloaders else [] # users who can get & download file without url shortner
lazy_renamers = [int(lazrenamers) if id_pattern.search(lazrenamers) else lazrenamers for lazrenamers in environ.get('LAZY_RENAMERS', '1989461858').split()]
...
LZURL_PRIME_USERS = [int(lazyurlers) if id_pattern.search(lazyurlers) else lazyurlers for lazyurlers in environ.get('LZURL_PRIME_USERS', '1989461858').split()]
...
UPI_ID = environ.get('UPI_ID', 'moviebotplan@upi') #enter your upi id here - grab it from your online payment methods.

# Others
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/real_MoviesAdda3/186') # Tutorial video link for opening shortlink website 
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', False))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'ynvmoviesrequest')
P_TTI_SHOW_OFF = is_enabled((environ.get('P_TTI_SHOW_OFF', "False")), False)
IMDB = is_enabled((environ.get('IMDB', "True")), True)
SINGLE_BUTTON = is_enabled((environ.get('SINGLE_BUTTON', "False")), False)
CUSTOM_FILE_CAPTION = environ.get("CUSTOM_FILE_CAPTION", "⚡<b>File uploaded by [Movies Adda™](https://ynvmovies.xyz/)</b>⚡\n\n📂<b>File Name:</b> ⪧ {file_caption} \n <b>Size: </b>{file_size}\n\n❤")
...
ADMIN_USRNM = environ.get('ADMIN_USRNM','benclass10') # WITHOUT @
MAIN_CHANNEL_USRNM = environ.get('MAIN_CHANNEL_USRNM','ynvmoviesupdates') # WITHOUT @
DEV_CHANNEL_USRNM = environ.get('DEV_CHANNEL_USRNM','ynvmoviesupdates') # WITHOUT @
LAZY_YT_HANDLE = environ.get('LAZY_YT_HANDLE','ynvmoviesupdates')  # WITHOUT @ [  add only handle - don't add full url  ] 
MOVIE_GROUP_USERNAME = environ.get('MOVIE_GROUP_USERNAME', "ynvmoviesrequest1") #[ without @ ]

# Url Shortner
URL_MODE = is_enabled((environ.get("URL_MODE","True")), False) # make it true to enable url shortner in groups or pm
URL_SHORTENR_WEBSITE = environ.get('URL_SHORTENR_WEBSITE', 'avhlinks.site') #Always use website url from api section 
URL_SHORTNER_WEBSITE_API = environ.get('URL_SHORTNER_WEBSITE_API', '858bed32cb3a9e68de45894f9fd76a86e655b9b0')
